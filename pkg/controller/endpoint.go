...
func (c *Controller) handleUpdateEndpoint(key string) error {
	...
	for _, vip := range vips {
		for _, port := range svc.Spec.Ports {
			...
			// chassis template variable name format: LB_BACKENDS_<PROTOCOL>_<IP_HEX>_<PORT>
			...
			if len(backends) != 0 {
				...
				klog.Infof("add vip %s with backends %v to LB %s", lbVIP, lbBackends, lb)
				if err = c.OVNNbClient.LoadBalancerAddVip(lb, lbVIP, lbBackends...); err != nil {
					klog.Errorf("failed to add vip %s with backends %s to LB %s: %v", lbVIP, lbBackends, lb, err)
					return err
				}
				if !ignoreHealthCheck && len(ipPortMapping) != 0 {
					klog.Infof("add health check ip port mapping %v to LB %s", ipPortMapping, lb)
					if err = c.OVNNbClient.LoadBalancerAddHealthCheck(lb, lbVIP, ignoreHealthCheck, ipPortMapping, externals); err != nil {
