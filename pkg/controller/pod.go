...
func (n *NamedPort) AddNamedPortByPod(pod *v1.Pod) {
	...
	sidecarContainers := make([]v1.Container, 0, len(pod.Spec.InitContainers))
	for i := range pod.Spec.InitContainers {
		if pod.Spec.InitContainers[i].RestartPolicy != nil &&
			*pod.Spec.InitContainers[i].RestartPolicy == v1.ContainerRestartPolicyAlways {
			sidecarContainers = append(sidecarContainers, pod.Spec.InitContainers[i])
		}
	}

	containers := slices.Concat(sidecarContainers, pod.Spec.Containers)
	if len(containers) == 0 {
		return
	}
