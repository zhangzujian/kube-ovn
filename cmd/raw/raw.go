...
func (c RawConn) ReadPacketConn(conn net.PacketConn, timeout time.Duration) ([]byte, net.Addr, error) {
	...
	case <-time.After(timeout):
		return nil, nil, nil
	case pkt := <-c.packetSource.Packets():
...
func main() {
	...
	filter := "ether[0:4] = 0xffffffff and udp and dst port 53"
	...
	wconn, err := packet.Listen(ifi, packet.Raw, unix.ETH_P_ALL, nil)
	if err != nil {
		panic(err)
	}
	...
	svr := &dns.Server{
		...
		Handler:        mux,
		DecorateReader: func(r dns.Reader) dns.Reader { return &RawConn{Reader: r, packetSource: source} },
		UDPSize:        1024,
