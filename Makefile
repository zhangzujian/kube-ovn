...
build-go:
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build $(GO_BUILD_FLAGS) -buildmode=pie -o $(CURDIR)/dist/images/vpcnatgateway/send-rarp-request -v ./cmd/send-rarp-request
...
build-go-arm:
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build $(GO_BUILD_FLAGS) -buildmode=pie -o $(CURDIR)/dist/images/vpcnatgateway/send-rarp-request -v ./cmd/send-rarp-request
