...
func TestDemo(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("new case runner error: %v", err)
	}
	...
	for i := 0; i < 3; i++ {
		...
		if s.Success {
			if len(s.Records) != 1 {
				t.Errorf("record count should be 1, but got %d", len(s.Records))
				continue
			}
			if s.Records[0].Success && s.Records[0].Elapsed < 100 {
				time.Sleep(time.Duration((100 - s.Records[0].Elapsed)) * time.Millisecond)
			}
		}
	}
