...
func (c *SubnetClient) CreateSync(subnet *apiv1.Subnet, timeout time.Duration) *apiv1.Subnet {
	s := c.Create(subnet)
...
// If the subnet doesn't become ready before the timeout, it will fail the test.
func (c *SubnetClient) PatchSync(original, modified *apiv1.Subnet, timeout time.Duration) *apiv1.Subnet {
...
func (c *SubnetClient) Delete(name string, options metav1.DeleteOptions) {
	err := c.SubnetInterface.Delete(context.TODO(), name, options)
...
func (c *SubnetClient) DeleteSync(name string, options metav1.DeleteOptions, timeout time.Duration) {
