...
func PatchPod(cs clientset.Interface, original, modified *corev1.Pod, timeout time.Duration) *corev1.Pod {
	patch, err := util.GenerateMergePatchPayload(original, modified)
	ExpectNoError(err)

	var patchedPod *corev1.Pod
	err = wait.PollImmediate(2*time.Second, timeout, func() (bool, error) {
		p, err := cs.CoreV1().Pods(original.Namespace).Patch(context.TODO(), original.Name, types.StrategicMergePatchType, patch, metav1.PatchOptions{}, "")
		if err != nil {
