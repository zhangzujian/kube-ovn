...
var _ = framework.Describe("[group:underlay]", func() {
	...
	ginkgo.BeforeEach(func() {
		...
		itFn = func(exchangeLinkName bool) {
			...
			pn = providerNetworkClient.CreateSync(pn, timeout)
	...
	ginkgo.AfterEach(func() {
		...
		providerNetworkClient.DeleteSync(providerNetworkName, metav1.DeleteOptions{}, timeout)
	...
	framework.ConformanceIt("should keep pod mtu the same with node interface", func() {
		...
		_ = providerNetworkClient.CreateSync(pn, timeout)
		...
		_ = subnetClient.CreateSync(subnet, timeout)
		...
		podClient.DeleteSync(podName, metav1.DeleteOptions{GracePeriodSeconds: new(int64)}, timeout)
	})
})
